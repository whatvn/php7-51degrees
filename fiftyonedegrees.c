...
static void fiftyonedegrees_obj_free(zend_object *object) {
    ...
    if (ffdegrees->user_agent){
//        free(ffdegrees->user_agent);
        ffdegrees->user_agent = NULL;
    }
    zend_object_std_dtor(&ffdegrees->std TSRMLS_CC);
    efree(ffdegrees);
}
/*
...
static void fiftyonedegreesMatch_obj_free(zend_object *object TSRMLS_CC) {
    ...
    zend_object_std_dtor(&ffdegrees_workset->std TSRMLS_CC);
...
PHP_FUNCTION(setUserAgent) {
    ...
    ffdegrees->user_agent = user_agent;
    RETURN_TRUE;
}
...
PHP_METHOD(fiftyonedegrees, get_value) {
    ...
    fiftyoneDegreesWorkset* ws = fiftyoneDegreesProviderWorksetGet(FIFTYONEDEGREES_G(provider));
    fiftyoneDegreesMatch(ws, ffdegrees->user_agent);
